...
async def generate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Generate a Suolala character image via OpenRouter image API."""
    remember_chat(update)
    ...
    try:
        ...
        # Step 2: call OpenRouter API (60s — image gen is slow)
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # Read raw text first — safer than resp.json() if response is not valid JSON
                raw_text = await resp.text()
                print(f"[GENERATE] HTTP {resp.status} | raw: {raw_text[:800]}")
                ...
                try:
                    ...
                    await status_msg.edit_text(
                        f"❌ Generation failed\n\n🔍 Bad JSON: {raw_text[:300]}"
                    )
                    return

        print(f"[GENERATE] Parsed response: {data}")

        # Handle error returned inside a 200 response (OpenRouter does this)
        if "error" in data:
            err = data["error"]
            err_msg = err.get("message", str(err)) if isinstance(err, dict) else str(err)
            print(f"[GENERATE] API-level error: {err_msg}")
            await status_msg.edit_text(f"❌ Generation failed\n\n🔍 {err_msg[:300]}")
            return

        # Safely parse image data list
        data_list = data.get("data") or []
        if not data_list:
            print(f"[GENERATE] Empty data list. Full response: {data}")
            await status_msg.edit_text(f"❌ Generation failed\n\n🔍 Empty data: {str(data)[:300]}")
            return
        ...
        if image_url:
            # Step 3a: send the image via URL
            await update.message.reply_photo(
                photo=image_url,
                caption=f"🎨 Suolala Girl{' — ' + extra if extra else ''}",
            )
        elif image_base64:
            # Step 3b: send the image via base64 decode
            ...
            print(f"[GENERATE] No url/b64_json in item: {image_item}")
            await status_msg.edit_text(f"❌ Generation failed\n\n🔍 {str(image_item)[:300]}")
            return
        ...
        print("[GENERATE] Request timed out (>60s)")
