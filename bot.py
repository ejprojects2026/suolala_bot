        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    ...
                    print(f"[GENERATE] API error {resp.status}: {error_body}")
                    await status_msg.edit_text("❌ Generation failed")
                    return

                data = await resp.json()
                print(f"[GENERATE] DEBUG response: {data}")

        # Parse image URL or base64 from response
        image_data = data.get("data", [{}])[0]
        image_url = image_data.get("url")
        image_base64 = image_data.get("b64_json")

        if image_url:
            # Step 3: send the image (URL)
            await update.message.reply_photo(
                photo=image_url,
                caption=f"🎨 Suolala Girl{' — ' + extra if extra else ''}",
            )
        elif image_base64:
            # Step 3: send the image (base64)
            ...
            print(f"[GENERATE] FULL ERROR: {data}")
            await status_msg.edit_text("❌ Generation failed")
            return

        # Step 4: edit status message
