...
async def generate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    status_message = await update.message.reply_text("🎨 Generating Suolala girl...")
    
    try:
        user_input = " ".join(context.args) if context.args else ""
        
        BASE_PROMPT = "cute 3D cartoon girl character, chibi style, big round brown eyes, smooth soft skin, small nose, soft smile, long straight black hair, center part, glossy hair, wearing purple to teal gradient hoodie with small white S logo on left chest, zipper hoodie, clean minimal style, matching teal pants, sneakers, full body, front view, symmetrical pose, soft studio lighting, clean background, high quality 3D render, pixar style, consistent character design"
        NEGATIVE_PROMPT = "ugly, realistic, blurry, low quality, bad anatomy, extra fingers, messy"
        
        full_prompt = f"{BASE_PROMPT} {user_input} {NEGATIVE_PROMPT}"
        encoded_prompt = urllib.parse.quote(full_prompt)
        
        image_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}"
        
        await update.message.reply_photo(photo=image_url)
        await status_message.edit_text("✅ Generated")
        
    except Exception as e:
        print("Generate ERROR:", e)
        await status_message.edit_text("❌ Failed")
